...
const validatedFld = (el, condition) => {
    if (condition) {
    canSubmit = true;
  ...
  {
  evt.preventDefault();
  validatedFld(this, evt.target.reportValidity() );
  }
  );
  ...
  {
  evt.preventDefault();
  validatedFld(this, passwordRegEx.test(evt.target.value) );
  }
  );
  
  repasswordFld.addEventListener('keyup', function (evt) {
    evt.preventDefault();
    validatedFld(this, passwordFld.value === evt.target.value);
  });
